import React, {Component} from 'react';
import {SECTIONS, PRODUCTS} from "../context";
import {getPrice, slugify} from "../utils";
import { Link, DirectLink, Element, Events, animateScroll as scroll, scrollSpy, scroller } from 'react-scroll'
import Slider from "react-slick";
import {NavLink} from "react-router-dom";


function PrevArrow(props) {
  const { onClick } = props;

  return (
    <div className={'arrow arrow-prev arrow-grey'} onClick={onClick} />
  );
}

function NextArrow(props) {
  const { onClick } = props;

  return (
    <div className={'arrow arrow-next arrow-grey'} onClick={onClick} />
  );
}


export default class Index extends Component {

  componentDidMount(){
    document.dispatchEvent(new Event('MainPageHasBeenRendered'));
    document.addEventListener('ChangeCurrency', ()=>this.setState({}));
  }

  render(){
    const settings = {
      speed: 500,
      slidesToShow: 2,
      slidesToScroll: 2,
      infinite: true,
      nextArrow: <NextArrow/>,
      prevArrow: <PrevArrow/>
    };

    return (
      <div className={'content'}>
        <div className={'main-page'}>
          <div className={'btn-container'}>
            <a href={'https://api.whatsapp.com/send?phone=375293648560'}>
              <div className={'btn btn-inv'}>ASK US ANYTHING</div>
            </a>
            <a href={'https://www.instagram.com/duck_story_store/'} className={'social-icon-white'} target="_blank" style={{margin: '0 20px 0 30px'}}>
              <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="20" cy="20" r="19.5" stroke="#BDBDBD"/>
                <path d="M24.3802 30.9999H15.6225C13.8528 30.9999 12.1903 30.3102 10.94 29.0599C9.68973 27.8096 9 26.1471 9 24.3774V15.6225C9 13.8528 9.68973 12.1903 10.94 10.94C12.1903 9.68973 13.8528 9 15.6225 9H24.3802C26.1498 9 27.8123 9.68973 29.0626 10.94C30.313 12.1903 31.0027 13.8528 31.0027 15.6225V24.3802C31.0027 26.1498 30.313 27.8123 29.0626 29.0627C27.8123 30.3102 26.1498 30.9999 24.3802 30.9999ZM15.6253 10.8164C12.9763 10.8164 10.8191 12.9708 10.8191 15.6225V24.3802C10.8191 27.0292 12.9735 29.1863 15.6253 29.1863H24.3829C27.0319 29.1863 29.1891 27.0319 29.1891 24.3802V15.6225C29.1891 12.9735 27.0347 10.8164 24.3829 10.8164H15.6253Z" fill="white"/>
                <path d="M20.0027 25.614C16.9085 25.614 14.3887 23.0969 14.3887 19.9999C14.3887 16.9058 16.9058 14.3859 20.0027 14.3859C23.0996 14.3859 25.6167 16.903 25.6167 19.9999C25.6167 23.0941 23.0969 25.614 20.0027 25.614ZM20.0027 16.2023C17.9088 16.2023 16.2051 17.906 16.2051 19.9999C16.2051 22.0939 17.9088 23.7976 20.0027 23.7976C22.0966 23.7976 23.8003 22.0939 23.8003 19.9999C23.8003 17.906 22.0966 16.2023 20.0027 16.2023Z" fill="white"/>
                <path d="M25.7876 15.2927C26.3826 15.2927 26.8648 14.8105 26.8648 14.2156C26.8648 13.6206 26.3826 13.1384 25.7876 13.1384C25.1927 13.1384 24.7104 13.6206 24.7104 14.2156C24.7104 14.8105 25.1927 15.2927 25.7876 15.2927Z" fill="white"/>
              </svg>
            </a>
            <a href={'https://www.facebook.com/DUCK-STORY-1941758935885615/'} className={'social-icon-white'} target="_blank">
              <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="20" cy="20" r="19.5" stroke="#BDBDBD"/>
                <path fillRule="evenodd" clipRule="evenodd" d="M17.2402 31V20.7286H14.4517V16.9746H17.2402C17.2402 16.9746 17.2433 15.9202 17.2402 15.5677C17.2399 15.5401 17.2396 15.5123 17.2393 15.4841C17.2258 14.1704 17.2054 12.1794 18.8409 10.5719C20.4257 9.01246 22.255 9.00293 23.4778 8.99976H25.5072V12.849C25.5072 12.849 23.6017 12.541 22.6108 13.0269C21.5659 13.5414 21.3277 14.6879 21.3277 15.5582V16.9746H25.5104V20.7286H21.3245V31H17.2402Z" fill="white"/>
              </svg>
            </a>
          </div>
        </div>
        {
          SECTIONS.filter(section=>section.is_menu).map(section=>
            <Element key={section.id} id={slugify(section.name)} name={slugify(section.name)} className={'section'}>
              <a id={slugify(section.name)}/>
              <div className={'section-container'}>
                <div className={'section-name'}>{section.name}</div>
                <hr/>
                <div className={'section-products'}>
                  <Slider {...settings}>
                    {
                      section.products.map(product_id=>{
                        let product = PRODUCTS.find(obj=>obj.id===product_id);
                        return (
                          <div key={product_id} className={'product'}>
                            <div className={'product-img'}>
                              <NavLink to={`/product/${product_id}`}>
                                <img src={product.main_image?product.main_image.image:''} title={product.main_image?product.main_image.name:''} width={436} height={470}/>
                              </NavLink>
                            </div>
                            <div className={'product-name'}>
                              <NavLink to={`/product/${product_id}`}>{product.name}</NavLink>
                            </div>
                            <div className={'product-price'}>
                              <NavLink to={`/product/${product_id}`}>{getPrice(product)}</NavLink>
                            </div>
                          </div>
                        )
                      })
                    }
                  </Slider>
                </div>
              </div>
            </Element>
          )
        }
      </div>
    );
  }
}